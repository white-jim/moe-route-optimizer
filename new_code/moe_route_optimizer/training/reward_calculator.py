...
class RewardCalculator:
    ...
    def __init__(self, config: RewardConfig):
        ...
        # 数值稳定项：避免 batch-level baseline 精度为 0 时奖励爆炸
        self._min_accuracy_baseline = 0.05
        self._min_latency_baseline = 1e-6

    def _get_safe_baselines(self) -> Tuple[float, float]:
        """返回数值稳定后的基线，避免分母过小导致奖励爆炸。"""
        latency_baseline = max(float(self.baseline_latency), self._min_latency_baseline)
        accuracy_baseline = max(float(self.baseline_accuracy), self._min_accuracy_baseline)
        return latency_baseline, accuracy_baseline
    ...
    def compute(self, comm_delay: float, accuracy: float) -> Tuple[float, Dict[str, float]]:
        ...
        latency_baseline, accuracy_baseline = self._get_safe_baselines()
        
        # 1. 时延奖励：时延减少越多，奖励越高
        latency_reduction = (latency_baseline - comm_delay) / latency_baseline
        latency_reward = latency_reduction  # 范围大约在 [-1, 1]
        
        # 2. 精度奖励：精度保持得越好，奖励越高
        accuracy_ratio = accuracy / accuracy_baseline
        accuracy_reward = accuracy_ratio - 1.0  # 0 表示与基线相同，负值表示下降
        
        # 3. 精度惩罚：如果精度低于阈值，额外惩罚
        penalty = 0.0
        accuracy_threshold = accuracy_baseline * self.config.accuracy_penalty_threshold
    ...
    def compute_batch(self, comm_delays: torch.Tensor, 
        ...
        # 时延奖励
        latency_reduction = (latency_baseline - comm_delays) / latency_baseline
        latency_reward = latency_reduction
        
        # 精度奖励
        accuracy_ratio = accuracies / accuracy_baseline
