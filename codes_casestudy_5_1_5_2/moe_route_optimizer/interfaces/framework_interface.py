...
class InferenceFrameworkInterface(ABC):
    ...
    def set_analysis_enabled(self, enabled: bool = True):
        """Enable optional route/latency tracing if supported by the framework."""
        raise NotImplementedError("Analysis tracing is not supported by this framework")

    def clear_analysis_buffers(self):
        """Clear optional analysis buffers if supported by the framework."""
        raise NotImplementedError("Analysis tracing is not supported by this framework")

    def get_analysis_payload(self) -> List[Dict[str, Any]]:
        """Return framework-specific analysis payloads from the latest run."""
        return []

    def get_route_traces(self) -> List[Dict[str, Any]]:
        """Return token-to-expert routing traces from the latest run."""
        return []

    def get_latency_breakdown(self) -> List[Dict[str, Any]]:
        """Return per-layer timing breakdowns from the latest run."""
        return []

    def get_last_input_ids(self) -> List[torch.Tensor]:
        """Return tokenized inputs from the latest run when available."""
        return []
