...
4. ARCEvaluator: ARC 多项选择题评估器
5. LogiQAEvaluator: LogiQA 逻辑推理多项选择题评估器
6. GSM8KEvaluator: GSM8K 数学问答评估器
7. LAMBADAOpenAIEvaluator: LAMBADA 续写最后一个词评估器
8. DummyEvaluator: 虚拟评估器（用于测试）
"""
...
class BaseEvaluator(AccuracyEvaluatorInterface):
    ...
    def _load_from_hf_cache(self):
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to load dataset with datasets.load_from_disk: {exc}. "
                "Falling back to Arrow file loading."
            )
            arrow_file = self._find_arrow_file()
            if arrow_file:
                self._load_from_arrow(arrow_file)
                ...
                raise ValueError(
                    f"Cannot load HuggingFace cache via load_from_disk or Arrow fallback: {self.dataset_path}"
                ) from exc
    ...
    def _find_arrow_file(self) -> Optional[str]:
        ...
        # 优先匹配 split 子目录中的 arrow 文件
        for root, dirs, files in os.walk(self.dataset_path):
            root_lower = root.lower()
            if split_lower not in root_lower:
                continue
...
class ARCEvaluator(BaseEvaluator):
    """
    ARC 数据集评估器
    ARC 是多项选择题数据集，标准答案字段通常为 answerKey，选项在 choices 中。
    """
    ...
    def _extract_choices(self, doc: Dict) -> List[Tuple[str, str]]:
        choices = doc.get("choices", {})
        ...
        if isinstance(choices, dict):
            ...
            return [
                (str(label).strip().upper(), str(text))
        ...
        if isinstance(choices, list):
            ...
            for idx, choice in enumerate(choices):
                if isinstance(choice, dict):
                    label = str(
                        choice.get("label", self.CHOICE_LABELS[idx] if idx < len(self.CHOICE_LABELS) else idx)
                    ).strip().upper()
                    text = str(choice.get("text", choice.get("content", "")))
                else:
                    label = self.CHOICE_LABELS[idx] if idx < len(self.CHOICE_LABELS) else str(idx)
                    text = str(choice)
    ...
    def _build_prompt(self, doc: Dict) -> str:
        question = doc.get("question", "")
        choices = self._extract_choices(doc)
        options_text = "\n".join(f"{label}. {text}" for label, text in choices)

        return (
            "Answer the following multiple-choice science question by replying with only "
            "the option letter.\n\n"
            f"Question: {question}\n\n"
            f"Options:\n{options_text}\n\n"
            "Answer:"
    ...
    def get_dataset_iterator(self, batch_size: Optional[int] = None) -> Iterator[Tuple[str, Any]]:
        ...
        while self._current_index < total_samples:
            ...
            answer = item.get("answerKey", item.get("answer", item.get("label")))
            input_text = self._build_prompt(item)
    ...
    def _normalize_ground_truth(self, ground_truth: Any) -> Optional[str]:
        if ground_truth is None:
            return None

        text = str(ground_truth).strip().upper()
        if not text:
            return None

        digit_to_label = {"1": "A", "2": "B", "3": "C", "4": "D", "5": "E"}
        return digit_to_label.get(text, text)

    def _extract_prediction(self, model_output: Any) -> Optional[str]:
        if model_output is None:
            return None

        output_text = str(model_output).strip().upper()
        letter_match = re.search(r"\b([A-E])\b", output_text)
        if letter_match:
            return letter_match.group(1)

        digit_match = re.search(r"\b([1-5])\b", output_text)
        if digit_match:
            return self._normalize_ground_truth(digit_match.group(1))
...
class LogiQAEvaluator(BaseEvaluator):
    """
    LogiQA 数据集评估器
    LogiQA 是带上下文的四选一逻辑推理数据集。
    """
    ...
    def _build_prompt(self, doc: Dict) -> str:
        ...
        options_text = "\n".join(
            f"{label}. {option}"
            for label, option in zip(self.CHOICE_LABELS, options)
        )

        return (
            "Read the context and answer the multiple-choice question by replying "
            "with only one letter: A, B, C, or D.\n\n"
            f"Context: {context}\n\n"
            f"Question: {query}\n\n"
            f"Options:\n{options_text}\n\n"
            "Answer:"
    ...
    def get_dataset_iterator(self, batch_size: Optional[int] = None) -> Iterator[Tuple[str, Any]]:
        ...
        while self._current_index < total_samples:
            ...
            answer = item.get("correct_option", item.get("answer", item.get("label")))
            input_text = self._build_prompt(item)
    ...
    def _normalize_ground_truth(self, ground_truth: Any) -> Optional[int]:
        ...
        text = str(ground_truth).strip().upper()
        if text in self.CHOICE_LABELS:
            return self.CHOICE_LABELS.index(text)
        if text.isdigit():
            return int(text)
        return None
...
class GSM8KEvaluator(BaseEvaluator):
    """
    GSM8K 数据集评估器（简单实现）
    GSM8K 是一个小学数学文本推理数据集，答案通常以自然语言推导结尾并包含最终数值。
    """
    ...
    def _build_prompt(self, doc: Dict) -> str:
        ...
        return (
            ...
            "Show your reasoning briefly and end with 'The answer is <number>'.\n\n"
            f"Question: {question}\n\n"
            "Answer:"
        )

    @classmethod
    def _normalize_number(cls, value: Any) -> Optional[str]:
        if value is None:
            return None
        ...
        match = cls.ANSWER_PATTERN.search(text)
        if match:
            text = match.group(1)
        else:
    ...
    def evaluate_single(self, model_output: Any, ground_truth: Any) -> float:
        predicted = self._normalize_number(model_output)
        expected = self._normalize_number(ground_truth)
...
class LAMBADAOpenAIEvaluator(BaseEvaluator):
    """
    LAMBADA(OpenAI) 评估器
    ...
    def __init__(self, dataset_path: str, split: str = "test"):
        super().__init__(dataset_path, split)
        self._dataset_name = "LAMBADAOpenAI"

    def get_dataset_name(self) -> str:
        return self._dataset_name

    @classmethod
    def _extract_last_word(cls, text: Any) -> Optional[str]:
        if text is None:
            return None
        ...
        matches = list(cls.WORD_PATTERN.finditer(raw_text))
        if not matches:
            return None
    ...
    @classmethod
    def _build_prefix(cls, text: str) -> Optional[str]:
        matches = list(cls.WORD_PATTERN.finditer(text))
        if not matches:
            return None
    ...
    def _build_prompt(self, prefix_text: str) -> str:
        return (
            "Complete the following passage with exactly the next word.\n\n"
            f"{prefix_text}\n\n"
            "Next word:"
        )
    ...
    def get_dataset_iterator(self, batch_size: Optional[int] = None) -> Iterator[Tuple[str, Any]]:
        ...
        while self._current_index < total_samples:
            ...
            text = item.get("text", "")
            answer = self._extract_last_word(text)
            prefix = self._build_prefix(text)
            if answer is None or prefix is None:
                continue

            yielded_count += 1
            yield self._build_prompt(prefix), answer
    ...
    def evaluate_single(self, model_output: Any, ground_truth: Any) -> float:
        predicted = self._extract_last_word(model_output)
        expected = self._extract_last_word(ground_truth)
...
def create_evaluator(
    ...
    **kwargs
    ...
    Args:
        dataset_name: 数据集名称 ("boolq", "hellaswag", "mmlu", "arc", "logiqa", "gsm8k", "lambada_openai", "dummy")
        dataset_path: 数据集文件路径或 HuggingFace datasets 缓存目录
        split: 数据集划分 (train/validation/test)
        **kwargs: 传递给评估器的额外参数
    ...
    elif dataset_name_lower == "arc":
        return ARCEvaluator(dataset_path, split=split)
    elif dataset_name_lower == "logiqa":
        return LogiQAEvaluator(dataset_path, split=split)
    elif dataset_name_lower == "gsm8k":
        effective_split = split if split != "validation" else "test"
        return GSM8KEvaluator(dataset_path, split=effective_split)
    elif dataset_name_lower == "lambada_openai":
        effective_split = split if split != "validation" else "test"
        return LAMBADAOpenAIEvaluator(dataset_path, split=effective_split)
    elif dataset_name_lower == "dummy":
        return DummyEvaluator()
    else:
        raise ValueError(f"Unknown dataset: {dataset_name}. Supported: boolq, hellaswag, mmlu, arc, logiqa, gsm8k, lambada_openai, dummy.")
