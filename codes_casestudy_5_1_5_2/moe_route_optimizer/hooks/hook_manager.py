...
class HookManager:
    ...
    def is_training(self, value: bool):
        ...
        if value:
            ...
            self.generator.eval()
        self._deterministic = not value
        self._collect_states = value
    ...
    def register_hook(self, target_module: nn.Module, module_name: str = "moe_block"):
        ...
        if hasattr(target_module, '_register_perturbation_generator'):
            ...
            if not self._is_training and self._collect_states:
                target_module._set_hook_analysis(
    ...
    def set_analysis_mode(self, deterministic: bool = True, collect_states: bool = True):
        """设置补充实验分析模式：不训练策略，但收集扰动决策状态。"""
        self._is_training = False
        self._deterministic = deterministic
        self._collect_states = collect_states
        self.generator.eval()
        if self._cross_process_mode and self._cross_process_proxy is not None:
            self._cross_process_proxy._set_hook_analysis(
